...
class ComplaintController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $complaint->complainer_id = Auth::id();
    ...
    public function workerDashboard()
    {
        try {
            ...
            $progressUpdates = ProgressUpdate::with(['complaint', 'user'])
    ...
    /**
     * Display the Complainer dashboard
     */
    public function complainerDashboard()
    {
        $user = Auth::user();

        // Fetch the 5 most recent complaints submitted by this user
        $recentComplaints = \App\Models\Complaint::with(['campus', 'complaintType'])
            ->where('complainer_id', $user->id)
            ->orderBy('created_at', 'desc')
            ->take(5)
            ->get();

        return Inertia::render('Complainer/Dashboard', [
            'recentComplaints' => $recentComplaints,
        ]);
    }
} 